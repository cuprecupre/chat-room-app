    ...
    function simulateRound(game) {
        ...
        voteCombinations.forEach((votes) => {
            ...
            Object.values(votes).forEach(
                (target) => (voteCounts[target] = (voteCounts[target] || 0) + 1)
            );
    ...
    function verifyInvariants(game) {
        ...
        try {
            ...
            if (game.winnerId === "impostor") {
                if (impostorScore !== 10)
                    throw new Error(`Impostor ganó con ${impostorScore} pts (debería ser 10)`);
                if (!game.playerBonus["impostor"])
                    throw new Error("Impostor ganó pero no tiene registro de bonus");
            }

            // 3. Si Amigos Ganan -> Verificar Bonus
            if (game.winnerId !== "impostor" && !game.winnerId) {
                // calculateRoundScores no setea winnerId explicitamente en el objeto game a veces en tests unitarios dependiendo del mock, pero aquí usamos lógica real
                ...
                if (perfectFriends.length > 0) {
                    ...
                    if (game.playerScores[winner] !== 10)
                        throw new Error(
                            `Ganador ${winner} tiene ${game.playerScores[winner]} pts, esperaba 10`
                        );
                } else {
                    // Nadie perfecto -> nadie tiene bonus
                    if (bonusReceivers.length > 0)
                        throw new Error("Nadie fue perfecto pero alguien recibió bonus");
                    ...
                    Object.entries(game.playerScores).forEach(([p, s]) => {
                        if (p !== "impostor" && s >= 10)
                            throw new Error(`Amigo ${p} tiene ${s} pts sin ser perfecto`);
                    });
                }
            }
