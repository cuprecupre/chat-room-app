...
class Game {
    constructor(hostUser, options = {}) {
        ...
        this.showImpostorHint =
            options.showImpostorHint !== undefined ? options.showImpostorHint : true; // Por defecto mostrar pista
    ...
    calculateStartingPlayer() {
        ...
        const eligiblePlayers = this.playerOrder.filter((uid) => this.roundPlayers.includes(uid));
        ...
        console.log(
            `[Game ${this.gameId}] Ronda ${this.roundCount}: Jugador inicial = ${startingPlayer?.name} (índice ${roundIndex} de ${eligiblePlayers.length} elegibles)`
        );
    ...
    selectImpostorWithLimit() {
        ...
        if (lastTwoImpostors.length === 2 && lastTwoImpostors[0] === lastTwoImpostors[1]) {
            ...
            console.log(
                `[Game ${this.gameId}] Jugador ${this.players.find((p) => p.uid === excludedPlayer)?.name} fue impostor las últimas 2 veces, será excluido`
            );
        }
    ...
    removePlayer(userId) {
        ...
        if (wasHost && this.players.length > 0) {
            ...
            const nextHostId = this.playerOrder.find((uid) =>
                this.players.some((p) => p.uid === uid)
            );
            if (nextHostId) {
                ...
                console.log(
                    `[Game ${this.gameId}] Host transferido a ${newHostInfo.name} (${nextHostId})`
                );
            }
        }
    ...
    startNewRound() {
        ...
        const impostorName =
            this.players.find((p) => p.uid === this.impostorId)?.name || "desconocido";
        console.log(
            `[Game ${this.gameId}] Ronda ${this.roundCount}: palabra='${this.secretWord}', categoría='${this.secretCategory}', impostor='${impostorName}' (${this.impostorId})`
        );
        console.log(
            `[Game ${this.gameId}] Historial de impostores:`,
            this.impostorHistory
                .slice(0, 3)
                .map((uid) => this.players.find((p) => p.uid === uid)?.name || uid)
        );
    ...
    castVote(voterId, targetId) {
        ...
        console.log(
            `[Game ${this.gameId}] ${voterId} ${isChangingVote ? "cambió su voto a" : "votó a"} ${targetId}`
        );
    ...
    checkIfAllVoted() {
        const activePlayers = this.roundPlayers.filter(
            (uid) => !this.eliminatedInRound.includes(uid)
        );
    ...
    processVotingResults() {
        ...
        console.log(
            `[Game ${this.gameId}] Procesando resultados. Jugadores activos:`,
            activePlayers
        );
        ...
        if (mostVoted.length !== 1) {
            ...
            if (this.currentTurn >= this.maxTurns) {
                console.log(
                    `[Game ${this.gameId}] Vuelta 3 completada sin eliminación. ¡El impostor gana!`
                );
                ...
                this.lastRoundScores[this.impostorId] =
                    (this.lastRoundScores[this.impostorId] || 0) + 4;
    ...
    startNextTurn(wasTie = false) {
        console.log(
            `[Game ${this.gameId}] 🔄 startNextTurn llamado. Vuelta actual: ${this.currentTurn} → ${this.currentTurn + 1}`
        );
        console.log(
            `[Game ${this.gameId}] lastEliminatedInTurn antes de cambiar vuelta:`,
            this.lastEliminatedInTurn
        );
        ...
        if (this.currentTurn > 1 && !wasTie) {
            // No dar puntos si fue empate
            ...
            this.lastRoundScores[this.impostorId] =
                (this.lastRoundScores[this.impostorId] || 0) + points;
            console.log(
                `[Game ${this.gameId}] Impostor sobrevivió vuelta ${previousTurn}: +${points} puntos`
            );
        } else if (wasTie) {
        ...
        console.log(
            `[Game ${this.gameId}] ✅ Vuelta ${this.currentTurn} iniciada. lastEliminatedInTurn:`,
            this.lastEliminatedInTurn
        );
        this.persist();
    }
