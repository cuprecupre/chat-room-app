...
describe("InviteErrors Components", () => {
    describe("GameNotFoundCard", () => {
        it("renders correct title and subtitle", () => {
            ...
            expect(screen.getByText("La partida ya no existe")).toBeInTheDocument();
            expect(screen.getByText(/No encontramos esta partida/i)).toBeInTheDocument();
        });

        it("renders ONLY 'Volver al inicio' when onCreate is missing", () => {
            render(<GameNotFoundCard roomId="123" onCancel={() => { }} />);
            expect(screen.getByText("Volver al inicio")).toBeInTheDocument();
            expect(screen.queryByText("Crear nueva partida")).not.toBeInTheDocument();
        });
        ...
        it("calls correct callbacks on click", () => {
            ...
            fireEvent.click(screen.getByText("Crear nueva partida"));
