...
export default function DebugPreviewSingle() {
    ...
    return (
        <div className="min-h-screen bg-neutral-950 text-white font-sans">
            {/* Overlays de Ronda */}
            {viewId === 'r1' && <RoundStartOverlay state={{ ...mockStateCommon, phase: 'playing', currentRound: 1 }} />}
            {viewId === 'r2' && <RoundStartOverlay state={{ ...mockStateCommon, phase: 'playing', currentRound: 2 }} />}
            {viewId === 'r3' && <RoundStartOverlay state={{ ...mockStateCommon, phase: 'playing', currentRound: 3 }} />}

            {/* Resultados de Ronda */}
            {viewId === 'result_elim_host' && (
                <RoundResultOverlay state={mockStateResultElim} isHost={true} onNextRound={noop} currentUserId={currentUser.uid} />
            )}
            {viewId === 'result_elim_guest' && (
                <RoundResultOverlay state={mockStateResultElim} isHost={false} onNextRound={noop} currentUserId={guestUser.uid} />
            )}
            {viewId === 'result_tie_host' && (
                <RoundResultOverlay state={mockStateResultTie} isHost={true} onNextRound={noop} currentUserId={currentUser.uid} />
            )}
            {viewId === 'result_tie_guest' && (
                <RoundResultOverlay state={mockStateResultTie} isHost={false} onNextRound={noop} currentUserId={guestUser.uid} />
            )}

            {/* Game Over */}
            {viewId === 'gameover_friend_host' && (
                <GameOverScreen state={mockStateGameOverFriendWins} isHost={true} onPlayAgain={noop} user={currentUser} />
            )}
            {viewId === 'gameover_friend_guest' && (
                <GameOverScreen state={mockStateGameOverFriendWins} isHost={false} onPlayAgain={noop} user={guestUser} />
            )}
            {viewId === 'gameover_impostor_host' && (
                <GameOverScreen state={mockStateGameOverImpostorWins} isHost={true} onPlayAgain={noop} user={currentUser} />
            )}
            {viewId === 'gameover_impostor_guest' && (
                <GameOverScreen state={mockStateGameOverImpostorWins} isHost={false} onPlayAgain={noop} user={guestUser} />
            )}
            {viewId === 'gameover_tie' && (
                <GameOverScreen state={mockStateGameOverTie} isHost={true} onPlayAgain={noop} user={currentUser} />
            )}

            {/* Modals */}
            {viewId === 'modal_instructions' && (
                <InstructionsModal isOpen={true} onClose={noop} />
            )}
                ...
                <Modal isOpen={true} onClose={noop} title="¿Terminar partida?">
                    <div className="space-y-4">
                        <p className="text-neutral-300">
                            ¿Estás seguro de que deseas terminar la partida? Todos los jugadores serán expulsados.
                        </p>
                        <div className="flex gap-3">
                            <Button variant="ghost" className="flex-1" onClick={noop}>Cancelar</Button>
                            <Button variant="danger" className="flex-1" onClick={noop}>Terminar</Button>
                        </div>
                    </div>
                </Modal>
            )}
            {viewId === 'modal_leave' && (
                <Modal isOpen={true} onClose={noop} title="¿Abandonar partida?">
                    <div className="space-y-4">
                        ...
                        <div className="flex gap-3">
                            <Button variant="ghost" className="flex-1" onClick={noop}>Cancelar</Button>
                            <Button variant="danger" className="flex-1" onClick={noop}>Abandonar</Button>
                        </div>
                    </div>
                </Modal>
            )}
