...
export function ChatBubble({
    ...
    const renderContent = () => {
        ...
        if (isRevealed && text) {
            return (
                <div className={`px-4 py-2 ${isError ? 'text-red-400' : 'text-orange-400'} text-sm font-medium leading-relaxed`}>
                    {text}
                </div>
            );
        }
        ...
        <AnimatePresence mode="wait">
            ...
            >
                ...
                <div className={`absolute inset-0 z-0 ${isTyping ? 'opacity-100' : 'opacity-10'}`}>
                    {/* Main Body */}
                    <div className={`absolute inset-0 ${isError ? 'bg-red-500' : 'bg-orange-600'} rounded-2xl ${position === 'left' ? 'rounded-tl-none' : 'rounded-tr-none'}`} />

                    {/* Tail connected to body */}
                    <div
                        className={`
                            absolute top-0 w-3 h-3 ${isError ? 'bg-red-500' : 'bg-orange-600'}
                            ${position === 'left'
                                ? '-left-[11px]'
                                : '-right-[11px]'
                            }
